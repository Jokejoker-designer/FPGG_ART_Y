...
> **DEPRECATED — DO NOT USE AS ACTIVE ROADMAP.** Duplicate/historical copy. Authority: `Revised Arty A7 Program Master.md`. A7-LM-00/01/02/03 FROZEN; A7-LM-04 NEXT.
...
| **A7-LM-MAX** | configuration sweep; 8.45M tied / 10.55M untied are candidate probes | measured ceiling experiment |
...
## 2026-08-17 research reconciliation — contract-first roadmap

This Copy is the comparison and planning companion to the executable program. It was reconciled against the selected NotebookLM source set, but **local milestone contracts are the release authority**. A NotebookLM answer is research evidence or an engineering hypothesis; it cannot reopen a frozen release, close an open gate, or authorize an RTL change.

### Authority and non-negotiable boundaries

1. `docs/contracts/A7-LM-00.md`, `A7-LM-01.md`, and `A7-LM-02.md` are BOARD_PASS and frozen. Their bitstreams, numerical law, and claims must not be overwritten.
2. `docs/contracts/A7-LM-03.md` is OPEN. Its 25,088-parameter BRAM-resident model, `lm05-signsgd-v1`, and its conjunctive board gates control the next action. A7-LM-04 is not authorized until A7-LM-03 closes on the board.
3. The board, not the host, computes forward, CE/loss, backward, update, and next-token selection in validation evidence. The host may tokenize, stream a locked corpus, orchestrate, observe, and evaluate independently.
4. The DDR contract locks the unmodified official Digilent **AXI** MIG profile. `mig.prj` is not hand-edited; BIST and DMA are AXI masters. This resolves the earlier native-`app_*` recommendation in this Copy.
5. The base ladder is a self-scheduling RTL machine: no soft CPU, no Ethernet dependency, and no teacher dependency from A7-LM-00 through A7-LM-07.

### Evidence labels and decision rule

| Label | Meaning in this program | Consequence |
|---|---|---|
| **EVIDENCE** | A frozen contract, board receipt, measured counter, or a cited source that directly establishes a property | May support a bounded claim. |
| **ENGINEERING INFERENCE** | A design decision inferred from resource, bandwidth, or fixed-point constraints | Must name its metric and falsification test. |
| **NEEDS EXPERIMENT** | A capacity, throughput, convergence, or KD benefit not yet demonstrated on this board | Cannot be promised or used to close a milestone. |

The governing distinction is always:

```text
FITS != RUNS != TRAINS != CONVERGES != USEFUL
```

### Unified gated ladder

| Step | Current architectural intent | Advance only when all gates pass | Maximum honest claim |
|---|---|---|---|
| **A7-LM-03** | 25,088 parameters; persistent weights in BRAM by contract; 2 layers/2 heads | FPGA CE drop >= 30%, exact fold, every named bank changes, AFTER writes = 0, FPGA argmax, timing non-negative, frozen 00/01/02 hashes unchanged | First board-validated scaled multi-head/multi-layer native-training model. |
| **A7-LM-04** | Candidate 100K byte-level model; define its memory placement in a new contract only after 03 PASS | Fixed-point trajectory, held-out improvement, saturation telemetry, host-boundary audit, timing and reproducible artifact | First real-text/byte-level online-learning experiment, if the board evidence proves it. |
| **A7-LM-05** | Candidate 399K / 4-layer model | Causal-mask and all-layer gradient evidence, stable C=64 behavior, measured DDR/DMA stalls, held-out improvement | Four-layer research model, not a general-purpose LLM. |
| **A7-LM-06** | Candidate 803K lower-bound primary model | Full native learning, roofline counters, bandwidth/compute diagnosis, fixed-point convergence and freeze proof | Scaled native-training platform only after those results exist. |
| **A7-LM-07** | Candidate 1.495M primary model; DDR-resident trainable state | Independent held-out gain, retained baseline behavior, sustained tokens/s, power/thermal and immutable release evidence | Primary program success: a small FPGA-native autoregressive Transformer. |
| **A7-LM-08** | Candidate 4.276M stretch; hybrid DDR/BRAM KV only if its contract proves it necessary | DDR mixed-traffic stability, no DMA underflow/hazard, useful throughput, convergence, timing, and freeze | A measured DDR-dominated stretch result. |
| **A7-LM-MAX** | Parameter/configuration sweep; tied/untied 8.45M/10.55M are probes, not commitments | Record every attempted configuration, roofline, saturation, timing, convergence, and failure mode | The measured useful-training ceiling of this implementation. |

The architectural breakpoint is deliberate: BRAM can retain A7-LM-03 model state because that is its frozen contract. Later DDR-scaled profiles use BRAM for tiles, activations, partial sums, and the active KV window. The transition to hybrid KV at A7-LM-08 is an **engineering inference** that must be confirmed by measured BRAM pressure and DDR traffic; it is not a substitute for an A7-LM-03 board close.

### Bottleneck decision system

Every post-03 milestone records exactness, CE/NLL before and after training, saturation by tensor family, DDR read/write bandwidth, DMA wait/underflow/hazard counts, useful-MAC utilization, tokens/s, WNS/TNS, power, and immutable hashes. Diagnose before changing architecture:

```text
bit-exact mismatch      -> RTL/arithmetic/DMA correctness investigation
exact but CE stagnates  -> fixed-point law, corpus, or optimizer experiment
GEMM high / GEMV low U_MAC -> DDR-bound tile or scheduler experiment
low U_MAC in both modes -> control/FSM/DMA-overlap investigation
negative WNS or high routing use -> reduce/control-partition lanes before model scale-up
```

### Separate A7-KD research lane

KD is optional research, never a dependency of the base ladder. After A7-LM-03 BOARD_PASS, a frozen-regression and synthetic-target protocol study may begin without modifying the A7-LM contracts. Live single-teacher or multi-teacher experiments, provider transport, and any claim about knowledge transfer wait until A7-LM-07 BOARD_PASS.

Teacher outputs are data only. They may never write model/optimizer memory directly or issue executable board commands. A sparse soft-target experiment must log mapped probability mass (mean, p10, p50, p90), fallback rate to hard/sequence targets, teacher/top-K mass, renormalization, and held-out CE/PPL/calibration against a hard-target baseline across independent repeats. The project may claim useful fixed-point “dark knowledge” only if soft KD reproducibly beats that hard-target baseline on held-out data. Otherwise it may claim sparse-target hardware support only.

UART plus explicit credit/backpressure is the protocol proof path. Ethernet, multi-teacher arbitration, or Random Forest gating are post-primary A7-KD infrastructure experiments; they do not justify delaying a base LM gate.
...
| A7-LM-MAX tied candidate | 8,192 | 256 | 256 | 8 | 8 | 1,024 | **8,454,144** | 8.06 MiB | 1.00 MiB |
| A7-LM-MAX untied candidate | 8,192 | 256 | 256 | 8 | 8 | 1,024 | **10,551,296** | 10.06 MiB | 1.00 MiB |
...
│                  DDR DMA Scheduler                           │
│                         │                                    │
│              Official Digilent AXI MIG interface             │
│                         │                                    │
│                         ▼                                    │
│                   256 MB DDR3L                               │
...
The ladder uses the **official Digilent AXI MIG interface**. `docs/contracts/A7-LM-01.md` records the board preset as AXI, forbids hand-editing `mig.prj`, and assigns BIST/DMA the AXI-master role. The program therefore does not fork a native-`app_*` path merely to remove an abstraction layer. Performance work belongs in burst scheduling, AXI-master DMA overlap, and BRAM ping/pong reuse under the frozen board-compatible configuration.
...
Use BRAM as a **scratchpad in DDR-scaled profiles**, with one explicit contract-bound exception: A7-LM-03 retains its 25 KiB persistent weight state in BRAM during its board close. Do not make a DDR model store a retroactive A7-LM-03 requirement. After the first post-03 DDR-scaled contract, BRAM is the scratchpad rather than the full model store:
...
| **A7-LM-01** | Digilent-preset AXI MIG; AXI-master DDR DMA; BIST/perf counters | calibration 100/100; zero DDR errors; ≥0.85 GB/s sequential read target; reset/recalibration PASS | MIG PRJ SHA, BIST logs, BW CSV, address-map JSON | 3–4 pw |
...
| DDR interface | **official Digilent AXI MIG; AXI-master DMA** | do not change `mig.prj`; optimize only from measured need |
...
| Soft CPU | **none through A7-LM-07** | only as a separately approved post-primary research deviation; never to repair an open base gate |
